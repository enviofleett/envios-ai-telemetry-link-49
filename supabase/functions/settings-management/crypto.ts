...
// Updated crypto implementation with working MD5 hashing for GP51 compatibility
import { crypto as cryptoStd } from "https://deno.land/std@0.208.0/crypto/crypto.ts";

export async function createHash(input: string): Promise<string> {
  try {
    console.log(`Creating hash for input of length: ${input.length}`);
    
    // Use MD5 hashing for GP51 compatibility
...
export async function hashMD5(text: string): Promise<string> {
  try {
    ...
    // Use a JavaScript-based MD5 implementation that's reliable across environments
    const md5Hash = await jsBasedMD5(text);
    console.log('MD5 hash generated successfully');
    return md5Hash;
...
// Reliable JavaScript-based MD5 implementation
async function jsBasedMD5(input: string): Promise<string> {
  // MD5 implementation that works in Deno Deploy
