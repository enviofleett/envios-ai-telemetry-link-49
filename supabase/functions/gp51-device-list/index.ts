...
serve(async (req) => {
  ...
  try {
    ...
    console.log("🔄 [gp51-device-list] Attempting to fetch device list from GP51 with corrected action...");

    // Use the corrected action name 'querymonitorlist' instead of 'querymonitorlist'
    const gp51Result = await fetchFromGP51({
      action: "querymonitorlist",  // Corrected action name
      session: session!,
      additionalParams: {
    ...
    console.log(`✅ [gp51-device-list] Successfully fetched data with corrected action: 'querymonitorlist'`);
