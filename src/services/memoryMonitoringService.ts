...
export class MemoryMonitoringService {
  ...
  private analyzeMemoryPatterns(): void {
    ...
    if (growth > this.alertThresholds.memoryLeakGrowth && timespan > 300000) { // 5 minutes
      this.triggerAlert({
        type: 'memory_leak',
        message: `Potential memory leak detected: ${(growth / 1024 / 1024).toFixed(2)}MB growth over ${(timespan / 60000).toFixed(1)} minutes`,
        metrics: newest,
        timestamp: Date.now()
      });
    }
    ...
    if (usagePercent > this.alertThresholds.highUsagePercent) {
      this.triggerAlert({
        type: 'high_usage',
        message: `High memory usage detected: ${usagePercent.toFixed(1)}% of heap limit`,
        metrics: newest,
        timestamp: Date.now()
      });
    }
...
export const memoryMonitor = new MemoryMonitoringService();
