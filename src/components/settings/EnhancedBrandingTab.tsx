...
import { Building2, Palette, Type, Eye } from 'lucide-react';
import UserSelector from '@/components/settings/UserSelector';
import BrandingSaveControls from '@/components/settings/BrandingSaveControls';

interface EnhancedBrandingTabProps {
  userId?: string;
}

const EnhancedBrandingTab: React.FC<EnhancedBrandingTabProps> = ({ userId: propUserId }) => {
  const [selectedUserId, setSelectedUserId] = useState<string | undefined>(propUserId);
  const { 
    settings, 
    isLoading, 
    isSaving, 
    ...
    discardChanges,
    getChangedFields
  } = useEnhancedBrandingSettings({ userId: selectedUserId });

  const handleUserSelect = (userId: string | undefined) => {
    if (hasUnsavedChanges) {
      const confirmed = window.confirm(
        'You have unsaved changes. Are you sure you want to switch users? Your changes will be lost.'
      );
      if (!confirmed) return;
    }
